...
public class ControladorDulce extends MouseAdapter {
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (e.getSource() == this.view.getBtnActualizar()) {
            int index = this.view.getTblDulces().getSelectedRow();
            Dulce temp = modelo.getDulceAddIndex(index);
            ...
            try{
                ...
                candy.setId(temp.getId());
                if (modelo.actualizarDatos(candy)){
                    ImageIcon icon = new ImageIcon("editar.png");
                    JOptionPane.showMessageDialog(view, "Se actualizo la base de datos correctamente", "Actualizado", JOptionPane.INFORMATION_MESSAGE);
                    modelo.cargarDatos();
                    this.view.getTblDulces().setModel(modelo);
                    this.view.getTblDulces().updateUI();
                } else {
